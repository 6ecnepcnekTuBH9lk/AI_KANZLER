...
def main():
    ...
    for rownum, source in enumerate(baseline["rows"], 2):
        ...
        if rownum in range(2, 10):
            ...
            category, required, reason = (
                "SHOE_CONSTRUCTION",
                ["материал верха", "лодыжку", "длина стельки", "перфорациями"],
                "Верх не обозначен явно; нет высоты верха, длины стельки и специальных конструктивных признаков. Подкладка/стелька/подошва извлечены отдельно.",
            )
        elif rownum in (13, 18, 19):
            category, required, reason = (
                "UNKNOWN_FIBER",
                ["тенсел"],
                "Торговое название «тенсел» не заменяет документированный состав волокна.",
            )
            if rownum in (18, 19):
                difference = "E"
                reason += " Предыдущий Skill использовал недоказанную идентификацию волокна. Для выбора кода по-прежнему не хватает данных."
                if rownum == 18:
                    reason += " Код app_before 6103490002 дополнительно ошибочно относится к ветви брюк/прочих изделий, а не пиджаков."
                else:
                    reason += " Код app_before 6103490002 дополнительно пропускал предшествующую ветвь брюк и бриджей 6103490001."
        elif rownum in (14, 15):
            category, required, reason = (
                "DENIM_AND_USE",
